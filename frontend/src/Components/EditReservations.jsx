...
export default function EditReservations() {
  ...
  useEffect(() => {
    const fetchReservation = async () => {
      ...
      if (!token) {
        setGeneralError("You must be logged in to edit a reservation.");
        setFetching(false);
        return;
      }
      try {
        const res = await fetch(`http://localhost:8085/reservation/${reservationId}`, {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await fetch(`http://localhost:8085/reservation/edit/${reservationId}`, {
        method: "PUT",
        headers: {
  ...
  if (fetching) return <p style={{ color: "white", textAlign: "center" }}>Loading reservation details...</p>;
  ...
  return (
    ...
    >
      ...
      >
        ...
        />
        {checkInDateError && <p style={{ color: "red", marginTop: 4 }}>{checkInDateError}</p>}
        ...
        <input
          ...
          style={{ width: "95%", padding: "8px", marginTop: 6, borderRadius: 6 }}
        />
        {checkOutDateError && <p style={{ color: "red", marginTop: 4 }}>{checkOutDateError}</p>}
        ...
        <input
          ...
          style={{ width: "95%", padding: "8px", marginTop: 6, borderRadius: 6 }}
        />
        {noOfGuestsError && <p style={{ color: "red", marginTop: 4 }}>{noOfGuestsError}</p>}

        <div style={{ marginTop: 6 }}>
          <small style={{ color: "#171717" }}>
            Max guests allowed for this room type: {ROOM_CAPACITY[roomType]}
          </small>
        </div>

        {generalError && <p style={{ color: "red", marginTop: 12 }}>{generalError}</p>}
